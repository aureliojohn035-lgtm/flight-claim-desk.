...
async function lookupFlightStatus(airline, flightNumber, date) {
  ...
  // and also calculates the real route distance from verified airport
  // coordinates. Inside the Claude artifact sandbox that route doesn't exist
  // (fetch fails to even reach it, or 404s), so this falls back to a
  // deterministic mock in that one specific case — meaning this same
  // component works unchanged in both environments. Any OTHER kind of
  // failure (wrong key, wrong plan, provider error) is treated as a real
  // error and surfaced to the person, not silently hidden behind fake data.
  try {
    ...
    if (res.status === 404) {
      // The backend route itself doesn't exist in this environment — fall through to mock.
    } else if (res.ok) {
      ...
      // Route exists but returned a real error (bad key, wrong plan, provider
      // rejected the request, etc.) — this is a genuine problem, not a
      // "route doesn't exist" situation, so don't mask it with mock data.
      const errBody = await res.json().catch(() => ({}));
      return {
        found: false,
        backendError: true,
        message: errBody.message || "The flight verification service returned an error.",
        hint: errBody.hint,
      };
    }
  } catch {
    // fetch itself threw (e.g. network blocked entirely, as in the artifact
    // sandbox) — fall through to mock.
  }
...
export default function FlightClaimChecker() {
  ...
  async function handleVerify() {
    ...
    try {
      ...
      if (info.backendError) {
        setVerifyErrorDetail({ message: info.message, hint: info.hint });
      ...
      if (info.dateMismatch && info.actualDate) {
        setDateWarning(
          `This flight's actual date is ${info.actualDate}, not ${form.date}. The free plan can only verify real-time flights, so double-check this is the right one before using this result.`
        );
      }
      const isCancelled = info.status === "cancelled";
      setForm((f) => ({
        ...f,
        delayType: isCancelled ? "cancelled" : "delayed",
        hours: isCancelled ? f.hours : String(info.delayHours ?? 0),
      }));
      setVerifiedRoute(
        info.distanceKm
      ...
      <div className="w-full max-w-md">
        ...
        {step === "form" && (
          <form onSubmit={handleCheck}>
            ...
            {lookupState === "error" && (
              <div className="mb-6">
                <p className="text-xs" style={{ color: "#A85D50" }}>
                  {verifyErrorDetail
                    ? verifyErrorDetail.message
                    : "That flight couldn't be found. Double-check the details and try again."}
                </p>
                {verifyErrorDetail?.hint && (
                  <p className="text-xs mt-1" style={{ color: "#8A877E" }}>
                    {verifyErrorDetail.hint}
                  </p>
                )}
              </div>
            ...
            {lookupState === "done" && dateWarning && (
              <p className="text-xs mb-6" style={{ color: "#B08D3E" }}>
                {dateWarning}
              </p>
            )}
